...
This implementation was created for academic use, but can be [adapted] for comercial usage. 
...
- Easy to use code to test concepts and modify easyly
...
1. Random number initialization. After this, numbers between the interval [-1000, +1000] can be generated.

2. Definition of a vector of N size, which will be uses as decision variable vector.

3. Here is needed to instanciate the object NonLinearCG() and the folowing parameters iss obligatory:
 - error: used as a stop criterian to finish the iterative process of the method;
 - step: the known "alpha" variable used into the method in order to define the size of step of convergence; 
 - deltaX: the size used in the differenciation dy/dx;
 - x: the decision variable vector;
 - f: functin which one whants to find the minimum value. An example of sphere function could be something like this:
